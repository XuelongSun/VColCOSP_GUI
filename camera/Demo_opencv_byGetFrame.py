...
#############################################################################################################################
			...
			# cvImage = cv2.cvtColor(cvImage, cv2.COLOR_BGR2YUV_YV12)
##############################################################################################################################
			# graySize = c_char()
			# grayBuff = c_buffer(b'\0', imageParams.height * imageParams.width)
			# nRet = IMGCNV_ConvertToMono8(cast(userBuff, c_void_p), \
			# 							 byref(imageParams), \
			# 							 cast(grayBuff, c_void_p), \
			# 							 byref(graySize))
			# grayByteArray = bytearray(grayBuff)
			# cvImage = numpy.array(grayByteArray).reshape(imageParams.height, imageParams.width)
##############################################################################################################################
			# graySize = c_char()
			# grayBuff = c_buffer(b'\0', imageParams.height * imageParams.width)
