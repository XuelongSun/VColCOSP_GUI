...
class Controller:
    def __init__(self):
        ...
        ## results
        self.exp_results_update_timer = QTimer()
        self.exp_results_update_timer.timeout.connect(self.exp_update_results)
        ...
        for plot in self.viewer.plots:
            if plot.type == 'plot':
                ...
                for k, v in plot.bars.items():
                    ...
                    h, x = np.histogram(np.array([v_ for k_, v_ in d.items()]), bins=20)
    ...
    def serial_run_capture(self, raw_d_q:Queue, robot_d_q:Queue):
                    ...
                    if d == b'\x01':
                        ...
                        if d == b'\xfe':
                            ...
                            if d == b'\xfe\x01':
                                ...
                                raw_d_q.put(d_display)
                                robot_d_q.put(self.serial_data_model.robot_data)

    def serial_update_robot_id(self):
        d_display = self.serial_raw_data_queue.get()
        # raw data display
    ...
    def serial_start_capture(self):
        if self.viewer.com.pb_start_capture.text() == 'Start \n Capture':
            if self.serial_port.isOpen():
                print('start capture robot data from serial port')
                self.viewer.system_logger('start capturing robot data...', out='com')
                self.serial_data_is_running = True
                self.serial_raw_data_queue = Queue()
                self.serial_robot_data_queue = Queue()
                self.process_robot_data_capture = Process(target=self.serial_run_capture,
                                                            args=(self.serial_raw_data_queue,
                                                                self.serial_robot_data_queue))
                self.process_robot_data_capture.start()
    ...
    def exp_setup_result_win(self):
        ...
        self.viewer.exp_results.et_title.setText('Experiment-{}'.format(name))
        self.viewer.exp_results.setup_figures([{'type':"plot", 
                                                "name": "prey energy", 
                                                'color':'c'},
                                               {'type':"plot",
                                               ...
                                               {'type':"distribution",
                                                "name":"f_avoid",
                                                'range':np.linspace(0.01, 10.01, 20),
                                                'color':'y',
                                                'width':0.5},
                                               {'type':"distribution",
                                                "name":"f_gather",
                                                'range':np.linspace(0.02, 0.32, 20),
                                                'color':'y',
                                                'width':0.3/20}])
