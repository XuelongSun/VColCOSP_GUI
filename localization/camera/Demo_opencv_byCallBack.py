...
g_Image_Grabbing_Timer=10  # unit : s
g_isStop=0
t_start = time.time()
t= time.time()
dataSize = 2304000
usr_image = c_buffer(b'\0', dataSize)
...
def onGetFrameEx(frame, userInfo):
	...
	# print("BlockId = %d userInfo = %s  time = %4f"  %(frame.contents.getBlockId(frame), c_char_p(userInfo).value, time.time()-t))
	...
	memmove(usr_image, c_char_p(imageBuff), imageParams.dataSize)
...
def demo():	
	...
	if ( nRet != 0 ):
		...
		# 设置曝光
	nRet = setExposureTime(camera, 8000)
	if ( nRet != 0 ):
	...
	t_while = time.time()
	while (time.time() - t_while < g_Image_Grabbing_Timer):
		grayByteArray = bytearray(usr_image)
		cvImage = numpy.array(grayByteArray).reshape(1200, 1920)
		cvImage = cv2.cvtColor(cvImage, cv2.COLOR_BAYER_BG2GRAY)
		show_Image = cv2.resize(cvImage, (960, 600))
		cv2.imshow('myWindow', show_Image)
		# gc.collect()
